...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		} else if (arg == "admintools") {
		    Lua* lua = DirectorManager::instance()->getLuaInstance();

            Reference<LuaFunction*> adminTools = lua->createFunction("AdminTools", "openToolsSUI", 0);
            *adminTools << creature;

            adminTools->callFunction();
		} else if (arg == "test") {
		    const int focusMod = creature->getSkillMod("force_focus");
            const int rd = creature->getSkillMod("ranged_defense");
            const int md = creature->getSkillMod("melee_defense");
            String message = "rd: " + String::valueOf(rd) + ", md: " + String::valueOf(md) + ", ff: " + String::valueOf(focusMod);
