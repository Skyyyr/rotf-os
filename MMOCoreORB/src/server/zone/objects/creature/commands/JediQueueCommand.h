...
public:
	...
	ManagedReference<Buff*> createJediSelfBuff(CreatureObject* creature) const {
        ...
        bool isForceFocus = skillMods.contains("forcefocus");

		for (int i=0; i < skillMods.size(); ++i) {
			int modValue = skillMods.elementAt(i).getValue();
			int frsModifiedValue = getFrsModifiedBuffValue(creature, modValue);
			String skillModName = skillMods.elementAt(i).getKey();
			//Sky 5/10/2024 -> This allows us to have 1 buff that scales with a specific mod
			if (isForceFocus && (skillModName != "forcefocus")) {
			    frsModifiedValue += creature->getSkillMod("force_focus");
            }
			buff->setSkillModifier(skillModName, frsModifiedValue);
		}
